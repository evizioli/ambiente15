...
<?php include_partial('bidg5i1s/assets') ?>
<?php use_stylesheets_for_form($filters) ?>
<?php use_javascripts_for_form($filters) ?>
<div class="container-fluid">
  <h1>ELEFANTE MARINO DEL SUR (Mirounga leonina)<br/><small class="text-muted">Indicador: Producción de crías</small></h1>
	...
	<div class="row">
	  ...
	  <div class="col-lg-4">
        ...
        <div class="bs_admin_filter">
          ...
          <form action="<?php echo url_for('bid_g5_i1_collection', array('action' => 'indicador')) ?>" method="post">
            ...
            <?php foreach ($configuration->getFormFilterFields($filters) as $name => $field): ?>
                <?php if ((isset($filters[$name]) && $filters[$name]->isHidden()) || (!isset($filters[$name]) && $field->isReal())) continue ?>
                  <?php include_partial('bidg5i1s/filters_field', array(
                    'name'       => $name,
            ...
            <div class="form-group row">
              <div class="offset-sm-2 col-lg-10">
                <?php echo link_to('<span class="glyphicon glyphicon-refresh" aria-hidden="true"></span>', 'bid_g5_i1_collection', array('action' => 'indicador'), array('query_string' => '_reset', 'method' => 'post', 'title'=>__('Reset', array(), 'sf_admin'))) ?>
      ...
      <tbody>
        <?php $anterior=null; foreach( $resultado as  $data): ?>
      	...
      	$actual =$data['crias']; 
      	 if($anterior ){
      	     if($anterior >$actual){
      	         if($anterior >0 && $actual/$anterior>.03) $clase='rojo';
      	     } else {
      	         if($actual>0 && $anterior/$actual>.03) $clase='rojo';
