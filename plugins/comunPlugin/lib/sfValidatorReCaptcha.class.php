...
class sfValidatorReCaptcha extends sfValidatorBase
{
  ...
  protected function doClean($value)
  {
    ...
    if (true !== ($answer = $this->check(array(
      'secret' => $this->getOption('private_key'),
      'response'   => $response,
    ))))
    {
  ...
  {
    if (false === ($fs = @fsockopen($this->getOption('server_host'), 443, $errno, $errstr, $this->getOption('server_timeout'))))
    {
    ...
    $verifyResponse = file_get_contents(
	    		sprintf(
    				"https://%s%s?%s",
    				$this->getOption('server_host'), 
    				$this->getOption('server_path'), 
    				$query
	    		)
    		);
  	...
  	$ob=get_object_vars($responseData);
  	return join(', ', $ob['error-codes']);
  }
}
