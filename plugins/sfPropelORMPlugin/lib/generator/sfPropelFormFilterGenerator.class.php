...
class sfPropelFormFilterGenerator extends sfPropelFormGenerator
{
  ...
  public function getWidgetOptionsForColumn(ColumnMap $column)
  {
    ...
    $withEmpty = !$column->isForeignKey() ? array("'with_empty' => false") : array();
    switch ($column->getType())
    {
      case PropelColumnTypes::BOOLEAN:
      case PropelColumnTypes::BOOLEAN_EMU:
        $options[] = "'choices' => array('' => '', 1 => 'Sí', 0 => 'No')";
        break;
      case PropelColumnTypes::DATE:
      case PropelColumnTypes::TIME:
      case PropelColumnTypes::TIMESTAMP:
        //$options[] = "'from_date' => new sfWidgetFormDate(), 'to_date' => new sfWidgetFormDate()";
        $options[] = "'from_date' => new sfWidgetFormBootstrapDatePicker(), 'to_date' => new sfWidgetFormBootstrapDatePicker(), 'template' => '<div style=\"width:18px; display: inline-block;\">De</div>%from_date%<br /><div style=\"width:18px; display: inline-block;\">A</div>%to_date%'";
